...
class PushInterpreter( breve.Object ):
	...
	def addInstruction( self, targetInstance, methodName, instructionName = "" ):
	...
	def readConfig( self, configFile ):
		...
		if ( not breve.breveInternalFunctionFinder.findFile( self, configFile ) ):
			print '''Could not locate Push configuration file %s''' % (  configFile )
			return
	...
	def run( self, codeObject, steps = -1.000000 ):
	...
	def runWithExecStackDump( self, codeObject, steps = -1.000000 ):
...
class PushProgram( breve.Object ):
	...
	def makeRandomCode( self, interpreter, size = -1.000000 ):
